...
import { isValidLineId, validateLineStateBody } from '../common/validation';
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const line = payload?.state?.reported?.status?.[`Line${lineId}`];

    const validated = validateLineStateBody(line);
    if (!validated.ok) {
      return errorJson(502, 'UPSTREAM_INVALID_STATE', `Invalid line state payload: ${validated.message}`, { event });
    }

    return json(200, validated.value);
  } catch {
    return errorJson(404, 'DEVICE_NOT_FOUND', 'Unable to read line state for device.', { event });
  }
};
