...
This document tells the **ESP32 firmware** team how to expose relay lines so the mobile app discovers them automatically. After this is implemented, adding line 4 (or more) is a **firmware-only** change — no mobile app update.

**Related docs:** [MOBILE_API.md](./MOBILE_API.md) (full HTTP contract), [API_ENDPOINTS.md](./API_ENDPOINTS.md) (endpoint reference).
...
The mobile discovery logic lives in `services/lineDiscovery.js` (`parseLineIdsFromStatus`).
